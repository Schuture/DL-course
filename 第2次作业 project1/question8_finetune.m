...
nHidden = [15]; % 每一层隐藏层的神经元数量，层数可以自己增加
...
yhat1 = MLPclassificationPredict(w, Xtest, nHidden, nLabels);
fprintf('Test error with final model = %f\n', sum(yhat1~=ytest)/t2);

% finetune，由于使用平方损失，使用最小二乘法解出最后一层的参数
x = question8_finetune_predict(w, Xtest, nHidden, nLabels);
yTestExpanded = linearInd2Binary(ytest,nLabels);
y = x * inv(x'*x)*(x'*yTestExpanded); % 直接用显式解
[v,yhat2] = max(y,[],2); % 最终y取的是每一个样本预测向量的最大值位置
fprintf('Test error with finetuned model = %f\n', sum(yhat2~=ytest)/t2);