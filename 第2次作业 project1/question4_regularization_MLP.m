function [f,g] = question4_regularization_MLP(w,X,y,nHidden,nLabels,lambda)
...
for i = 1:nInstances % 遍历每一个样本
    ...
    if nargout > 1
        ...
        if length(nHidden) > 1
            ...
            clear backprop
            for c = 1:nLabels
                % 用来反向传播的误差delta，sech双曲正割函数是双曲余弦的倒数，tanh激活函数的导数是后面括号中两项
...
g = g + lambda * w; % 给梯度加上l2正则项
f = f + 0.5 * norm(w); % 给损失函数加上l2正则项
