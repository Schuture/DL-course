...
def collate_fn(batch):
    '''
    Parameter: a batch of data
    Return: 
        image_paths: paths of images in this batch
        imgs: (torch.Tensor, bs * 512 * 512 *3) images in this batch
        score_maps: (torch.Tensor, bs * 128 * 128 * 1) score of every pixel in images
        geo_maps: (torch.Tensor, bs * 128 * 128 * 5) 4 distances of this pixel to bound, and 1 orientation of this bboxs
        training_masks: (torch.Tensor, bs * 128 * 128 * 1) mask of gt bbox
        mapping: (ndarray, n) the indices of texts and bboxes
        texts: (ndarray, n) the text of each bbox
        bboxs: (ndarray, n * 9) the coordinates of 4 bbox corners 
    '''
    ...
    for index, gt in enumerate(zip(transcripts, boxes)):
        for t, b in zip(gt[0], gt[1]): # 逐一提取transcript, box
    ...
    bboxs = np.stack(bboxs, axis=0) # add a new axis
